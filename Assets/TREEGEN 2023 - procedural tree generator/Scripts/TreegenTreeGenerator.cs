...
namespace Treegen
{
  ...
  public class TreegenTreeGenerator : MonoBehaviour
  {
    ...
    [Tooltip("The impact of noise functions at the direction of the branches")]
    [Min(0f)]
    public float RootNoiseForce = 1.5f;
    [Tooltip("Noise sets the function direction of the branches")]
    [Min(0f)]
    ...
    public AnimationCurve LeavesScaleCurve = new AnimationCurve(new Keyframe[2] { new Keyframe(0, 1, -0.5f, -0.5f), new Keyframe(1, 0, -0.5f, -0.5f) });
    ...
    bool GenLeaves(ref BranchProperties Branch, Vector3 From, Vector3 Normal, Vector3 Tangent, int Seg)
    {
      ...
      if (rez)
      {
        ...
        if (LeavesType == LeafType.Normal)
          GenLeavesNormal(LeavesDetail, ls * LeavesScale, From + ls * Normal * LeavesOffset * offset, Normal, Tangent);
        if (LeavesType == LeafType.Needle)
        {
          GenLeavesNeedle(LeavesDetail, ls * LeavesScale, From + ls * Normal * LeavesOffset * offset, Normal, Tangent);
        }
        if (LeavesType == LeafType.Palm)
          GenLeavesPalm(LeavesDetail, Parts, LeavesScaleCurve.Evaluate(1),ls * LeavesScale, From + ls * Normal * LeavesOffset * offset, Normal, Quaternion.AngleAxis(Seg * (180.0f / Parts), Normal) * Tangent, Twirl);
      }
      return rez;
    }
